...
from block_grasp_vision import (
    ...
    deproject_pixel,
    detector_box_center,
    render_depth_debug_image,
    render_yolo_center_debug_image,
...
def parse_args(argv):
    ...
    parser.add_argument('--show-rgb', action='store_true',
                        help='Show RGB image with YOLO box and center marker during block dry-run/debug.')
    parser.add_argument('--show-depth', action='store_true',
                        help='Show registered depth image with YOLO center marker during block dry-run/debug.')
    parser.add_argument('--depth-window-ms', type=int, default=3000,
                        help='How long to show the depth window in milliseconds.')
...
def require_block_args(args):
    ...
    numeric = (
        ...
        ('depth_window_ms', '--depth-window-ms'),
    ...
    if isinstance(args.depth_window_ms, bool) or args.depth_window_ms < 1:
        raise RuntimeError('--depth-window-ms must be a positive integer.')
...
def show_block_debug_windows(rgb_debug_image, depth_debug_image, milliseconds):
    try:
        import cv2
    except ImportError as exc:
        raise RuntimeError('OpenCV unavailable for debug display: {}'.format(exc))
    if isinstance(milliseconds, bool) or int(milliseconds) < 1:
        raise RuntimeError('--depth-window-ms must be a positive integer.')
    milliseconds = int(milliseconds)
    windows = []
    try:
        if rgb_debug_image is not None:
            rgb_window = 'Block RGB - YOLO box center'
            cv2.imshow(rgb_window, rgb_debug_image)
            windows.append(rgb_window)
        if depth_debug_image is not None:
            depth_window = 'Block registered depth - YOLO center'
            cv2.imshow(depth_window, depth_debug_image)
            windows.append(depth_window)
        if windows:
...
def localize_block(args, capture):
    """Ask the Python 3 detector, sample depth at YOLO box center, then deproject."""
    try:
        import cv2
        import numpy as np
    except ImportError as exc:
    ...
    try:
        ...
        center = detector_box_center(response['box'])
        debug = render_yolo_center_debug_image(
            capture['rgb'], response['box'], center, args.depth_radius)
        ...
        if getattr(args, 'show_depth', False):
            depth_debug = render_depth_debug_image(
                capture['depth'], center, capture['depth_encoding'],
                args.depth_min_m, args.depth_max_m, args.depth_radius)
        ...
        try:
            ...
            if getattr(args, 'show_rgb', False) or depth_debug is not None:
                show_block_debug_windows(
                    debug if getattr(args, 'show_rgb', False) else None,
                    depth_debug,
                    args.depth_window_ms)
            if (getattr(args, 'dry_run', False)
                    and getattr(args, 'tool_offset', None) is None
                    and not getattr(args, 'stop_at_pre_grasp', False)):
                return {
                    ...
                    'center_source': 'yolo_box',
                    'depth_ok': False,
                    'depth_error': str(exc),
                    'depth_m': None,
                    'depth_stats': None,
                    'undistorted_center': None,
                    'camera_xyz': None,
                    'rgb_header': copy.deepcopy(capture['rgb_header']),
                }
            raise RuntimeError(
                'depth_z=UNREADABLE target={} class={} yolo_center=({:.1f}, {:.1f}): {}'
                .format(
                    response['target'], response['class_name'],
                    center[0], center[1], exc))

        info = capture['camera_info']
        matrix = np.asarray(info.K, dtype=np.float64).reshape(3, 3)
        corrected_center = undistort_pixel(
            center[0], center[1], matrix,
...
def compute_block_context(args, arm):
    ...
    current_pose = arm.get_current_pose() if arm is not None else None
    if localization.get('depth_ok') is False:
        rospy.logwarn(
            'Block {} ({}) confidence={:.3f} yolo_center=({:.2f}, {:.2f}) '
            'depth_z=UNREADABLE reason={}'.format(
                localization['target'], localization['class_name'],
                localization['confidence'], localization['center'][0],
                localization['center'][1],
                localization.get('depth_error', 'unknown')))
        context = {
        ...
        if args.tool_offset is None and args.dry_run:
            rospy.logwarn(
                'Surface-only dry run: depth is unreadable, so base coordinate '
                'conversion is skipped.')
            publish_debug_geometry(args.base_frame, current_pose, None, None, None)
            return context
        raise RuntimeError(
            'Block depth is unreadable; no arm motion can be planned safely.')
    surface_camera = make_camera_point_pose(
        localization['rgb_header'], localization['camera_xyz'])
    rospy.loginfo(
        'Block %s (%s) confidence=%.3f yolo_center=(%.2f, %.2f) depth_z=%.4fm',
        localization['target'], localization['class_name'],
        localization['confidence'], localization['center'][0],
    ...
    if args.tool_offset is None and args.dry_run:
        ...
        return context
    if arm is None:
        raise RuntimeError('MoveIt arm is required once tool geometry or motion is requested.')
...
def is_surface_only_block_dry_run(args):
    return (
        args.mode == 'block_grasp'
        and args.dry_run
        and args.tool_offset is None
...
def main():
    ...
    try:
        if args.mode in ('home', 'grasp', 'place', 'pick_place', 'pick_lift_place',
                         'current_pose', 'wrist_forward') or (
                args.mode == 'block_grasp'
                and not is_surface_only_block_dry_run(args)):
        ...
        try:
            if args.mode == 'block_grasp':
                ...
                rospy.logerr(
                    'CRITICAL: operation interrupted, exiting non-zero.')
        except Exception:
            pass
        raise
    except Exception as exc:
