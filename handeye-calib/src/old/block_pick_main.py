...
def validate_runtime_args(args):
    ...
    if isinstance(args.depth_window_ms, bool) or args.depth_window_ms < 1:
        raise ValueError("--depth-window-ms must be a positive integer")
...
def build_child_command(args, request_fd, response_fd):
    ...
    command += [
        ...
        "--depth-window-ms", str(args.depth_window_ms),
    ]
    if args.show_rgb:
        command.append("--show-rgb")
    if args.show_depth:
        command.append("--show-depth")
    if args.dry_run:
...
def _remove_path_entries(raw_path, roots_to_remove):
    if not raw_path:
        return raw_path
    normalized_remove = set(
        os.path.abspath(entry) for entry in roots_to_remove if entry
    )
    kept = []
    for entry in raw_path.split(os.pathsep):
        ...
        if any(
            normalized_entry == root or normalized_entry.startswith(root + os.sep)
            for root in normalized_remove
        ):
            continue
        kept.append(entry)
...
def build_child_env(parent_env):
    """Run the ROS Python 2 child outside the active conda Python 3 env."""
    env = dict(parent_env)
    conda_prefix = env.get("CONDA_PREFIX")
    if conda_prefix:
        conda_bin = os.path.join(conda_prefix, "bin")
        env["PATH"] = _remove_path_entries(env.get("PATH", ""), [conda_bin])
        env["PYTHONPATH"] = _remove_path_entries(env.get("PYTHONPATH", ""), [conda_prefix])
    for key in list(env):
        if key.startswith("CONDA") or key in ("_CE_CONDA", "_CE_M", "PYTHONHOME"):
            env.pop(key, None)
    return env
