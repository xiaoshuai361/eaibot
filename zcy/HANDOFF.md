...
两者正式动作：先经过 A/B 共用抓取前中转点；A 点规划到 P 后方40mm，B 点规划到 P 后方30mm；在这里开启限位，再受保护地直线到 P，未触发才继续前探
两者限位最大前伸：65mm
限位路径步长：后方安全点到 P 为5mm，P 后继续前探为2mm
限位触发并开泵后：A 点沿原路径退到 P 后方40mm，B 点退到 P 后方30mm
```

预抓点后方安全距离用于进场过渡和吸附后的搬运间隙：A 点为 `40mm`，B 点为 `30mm`。限位未触发时也先直退到对应安全点，不把物块当作已吸住，也不进入 carry。
