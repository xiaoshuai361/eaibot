...
两者正式动作：先经过 A/B 共用抓取前中转点；A、B 两点都先规划到 P 后方30mm；若该普通规划失败，均以0.1秒间隔依次降级到 P 后方20mm、10mm、P。到达实际采用的起点后开启限位，再受保护地直线到 P，未触发才继续前探
两者限位最大前伸：65mm
限位路径步长：后方安全点到 P 为5mm，P 后继续前探为2mm
限位触发并开泵后：A、B 两点都沿原路径退到 P 后方30mm
```

预抓点后方安全距离用于首选进场过渡和吸附后的搬运间隙：A、B 两点均为 `30mm`。只有进场普通规划失败才使用 `20mm -> 10mm -> P` 降级；抓取完成或限位未触发后的撤退距离不降级，仍直退30mm，不把未触发的物块当作已吸住，也不进入 carry。
...
预抓点后方安全过渡/最大探测：A/B均为30mm/65mm
限位步长：安全点到 P 为5mm；P 后接触前探为2mm
成功后直退目标：A/B均为P后方30mm
...
无 Tag 投递顺序：楼宇同类框锁定并对准 -> 从机械臂当前姿态直接到仓内抓取点上方约 5cm（投递前不回零） -> 仓内抓取点 -> 开泵 -> 直线上升约 5cm -> 楼宇专用中转点 -> 共享 P 后方30mm -> 开限位 -> 5mm步长到P -> 2mm步长前探最多65mm -> 关泵等待0.7秒让物资贴稳 -> 立即直退30mm -> 自关泵起满3.0秒且直退成功后恢复巡线 -> 机械臂后台回idle。后台归位未完成时不得启动下一次投递，也不得停止机械臂公共进程。走满65mm未触发仍按用户决定强制释放并记成功；限位服务、串口或轨迹错误必须保持泵开启并返回失败。正式循迹投递直接使用任务摄像头的楼宇框宽估距，以“实测距离减450mm”沿相机前后方向修正P；框中心只负责进入红框后停车。四类楼宇共用在450mm参考距离下示教的ID1 power楼宇P和前探方向，只需示教一次；不再自动移动idle或辅助点，也不使用楼宇框计算示教高度和姿态。有 Tag 固定投递使用同一投递程序，因此投递前同样不回零。
