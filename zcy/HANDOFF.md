...
无 Tag 投递顺序：楼宇同类框锁定并对准 -> 启动回零 -> 仓内抓取点上方约 5cm -> 仓内抓取点 -> 开泵 -> 直线上升约 5cm -> 共享中转点 -> 该类 P 后方30mm -> 开限位 -> 5mm步长到P -> 2mm步长前探最多65mm -> 关泵等待0.7秒 -> 直退30mm -> idle。走满65mm未触发仍按用户决定强制释放并记成功；限位服务、串口或轨迹错误必须保持泵开启并返回失败。正式循迹投递直接使用任务摄像头的楼宇框宽估距，以“实测距离减450mm”沿相机前后方向修正P；框中心只负责进入红框后停车。四类P在车辆位于450mm参考距离时分别手动示教，命令只记录一个P的六关节角，并从已有手眼TF取得固定前后方向；不再自动移动idle或辅助点，也不使用楼宇框计算示教高度和姿态。
