...
左侧抓取区：A/B 共用 grasp_roi_ratio=(0.06,0.00,0.28,1.00)
...
-> 保持当前 idle，取得3个新鲜稳定定位观测
-> 普通关节规划到 A/B 共用的抓取前中转点
-> 普通规划到示教预抓点 P 后方40mm；失败时每隔0.1秒依次改试后方20mm、10mm、P
-> 在该后方安全点开启限位检测
...
无 Tag 使用“共享抓取前中转点 -> 首选 P 后方40mm（规划失败才降级20mm、10mm、P） -> 开启限位 -> 受保护地到 P -> 未触发再前探 -> P 后方40mm”；有 Tag 的首选距离为30mm并使用相同降级。降级只用于进场，撤退距离保持不变；总调度不得直接操作泵串口或修改限位时序。
