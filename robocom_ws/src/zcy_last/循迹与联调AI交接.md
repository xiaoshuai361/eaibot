...
B 点有 Tag 连续抓取已直接删除每个目标抓取前的启动回零逻辑，不使用跳过参数。A 点无 Tag 抓取保持原逻辑，`block_approach_staging` 在启动回零并等待关节状态稳定后执行。控制器若返回失败，先检查实际末端是否已在15mm/0.35rad容差内；已到达则接受，否则等待稳定并从最新状态重试一次。最终失败信息必须包含位置和姿态误差，便于区分“控制器误报已到位”和“机械臂确实未到位”。
