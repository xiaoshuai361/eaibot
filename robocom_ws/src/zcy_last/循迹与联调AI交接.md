...
正式有 Tag 和无 Tag 连续抓取都使用 `--skip-startup-home`，每个目标抓取前不回零。A 点 `block_approach_staging` 从当前姿态规划前先等待关节状态稳定。控制器若返回失败，先检查实际末端是否已在15mm/0.35rad容差内；已到达则接受，否则等待稳定并从最新状态重试一次。最终失败信息必须包含位置和姿态误差，便于区分“控制器误报已到位”和“机械臂确实未到位”。
