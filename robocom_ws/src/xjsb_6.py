...
def detect_and_visualize_fire(img, model):
    ...
    try:
        ...
        if not detections.empty:
            ...
            # 处理火灾框，忽略第6层的火灾
            for i, (fire_box, conf) in enumerate(fire_boxes_with_conf):
                x1_f, y1_f, x2_f, y2_f = fire_box

                # 确定当前火灾框所在的楼层
                fire_floor = None
                for floor, confidences in floor_fire_confidences.items():
                ...
                # 绘制火灾框
                info = fire_class_info['Fire']
                cv2.rectangle(vis_img, (x1_f, y1_f), (x2_f, y2_f), info['color'], 2)
                # 绘制火灾标签
                label = f"火灾:{conf:.2f}"
                text_pos = (x1_f, y1_f - 25) if y1_f > 25 else (x1_f, y2_f + 20)
                vis_img = cv2_add_fire_text(vis_img, label, text_pos, 16, info['color'])
...
def fire_detection(frame, save_dir="saved_fire_images"):
    ...
    try:
        ...
        if result[0] is not None:
            ...
            # 累加楼层统计（忽略第6层）
            for floor in range(1, 6):  # 只统计1-5层
...
class LaneFollower:
    def __init__(self):
        ...
        self.detection_intervals = [
            {"start": 31.5, "end": 32.5, "done": False, "type": "fire"},  # 第一次火灾检测
            {"start": 56, "end": 57, "done": False, "type": "people"},
            {"start": 67, "end": 69, "done": False, "type": "people"},
            {"start": 89.5, "end": 90.5, "done": False, "type": "fire"},  # 第二次火灾检测
            {"start": 102, "end": 103, "done": False, "type": "people"},
            {"start": 115.5, "end": 116.5, "done": False, "type": "rubbish"},  # 垃圾桶检测时段
            {"start": 127.5, "end": 128.5, "done": False, "type": "people"}
        ]
