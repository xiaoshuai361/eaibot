...
DRY_RUN = False           # 任务版直接发布速度；运行前确保车辆已放在起点并可随时急停。
...
FINAL_EXIT_TIME = 6.0     # 第九次右拐完成后继续直行时间(s)，随后停车结束。
...
ALIGN_LOCKED_ENTRY_ENABLED = True # 试验：入口摆正锁定首次横条角度，不再二次追新横条。
...
# ===== YOLO 模型识别 =====
YOLO_ENABLED = True          # 是否启用摄像头 0 的模型识别。
YOLO_STOP_ENABLED = True     # False 时只打印识别结果，不触发停车。
YOLO_CAMERA_INDEX = 0        # 模型识别摄像头编号。
YOLO_MODEL_PATH = "/home/eaibot/handeye-calib/src/model/yolov5/merge_yolov5n_320_best.onnx" # 真机默认 ONNX 模型文件。
YOLO_FRAME_INTERVAL = 1      # YOLO 线程每隔多少个摄像头 0 新帧做一次模型推理。
YOLO_CONFIDENCE = 0.60       # YOLO 置信度阈值。
YOLO_CENTER_BAND_RATIO = 0.80 # 目标框中心位于画面中间此比例时才触发停车。
YOLO_STOP_TIME = 1.0         # 模型触发后停车等待时间(s)。
YOLO_BUILDING_STOP_LIMIT = 1 # 每个路口内楼宇最多触发停车次数。
YOLO_PEOPLE_STOP_LIMIT = 2   # 每个路口内人群最多触发停车次数。
YOLO_PEOPLE_STOP_COOLDOWN = 6.0 # 人群两次停车之间的最小间隔(s)。
...
YOLO_BUILDING_MESSAGES = (
    ("Fire Building", "检测到火灾楼宇"),
    ("Collapsed Building", "检测到坍塌楼宇"),
    ("Toxic Gas-contaminated Building", "检测到有毒气体楼宇"),
    ("Electrical Fault Building", "检测到电力故障楼宇"),
)
YOLO_MODEL_PREFERRED_FILES = (
    "merge_yolov5n_320_best.onnx",
    "best.onnx",
)
YOLO_WINDOW_NAME = "line_cy_task_yolo" # 摄像头 0 模型识别调试窗口。
...
class YoloObstacleDetector(object):
    def __init__(self, model_path, confidence=YOLO_CONFIDENCE,
        ...
        self.model = None
        self.names = self._normalize_names(YOLO_CLASS_NAMES)
...
def locked_entry_alignment_plan(angle, now):
    angle = float(angle)
    now = float(now)
    if abs(angle) <= ALIGN_TOLERANCE_DEG:
        return {
            ...
            "angular": 0.0,
            "rotate_until": now,
            "settle_until": now + ALIGN_LOCK_SETTLE_TIME,
        }
    magnitude = clamp(abs(angle) * ALIGN_KP,
                      ALIGN_MIN_ANGULAR, ALIGN_MAX_ANGULAR)
    angular = -magnitude if angle > 0.0 else magnitude
    rotate_time = clamp(
...
class LaneFollower(object):
    def __init__(self):
        ...
        self.yolo_model_path = str(rospy.get_param(
        ...
        self.yolo_building_stop_limit = max(0, int(rospy.get_param(
            "~yolo_building_stop_limit", YOLO_BUILDING_STOP_LIMIT
        )))
        self.yolo_people_stop_limit = max(0, int(rospy.get_param(
            "~yolo_people_stop_limit", YOLO_PEOPLE_STOP_LIMIT
        )))
        self.yolo_people_stop_cooldown = max(0.0, float(rospy.get_param(
            "~yolo_people_stop_cooldown", YOLO_PEOPLE_STOP_COOLDOWN
        )))
        self.task_index = 0
        ...
        self.align_locked_angle = None
        self.align_locked_angular = 0.0
        self.align_locked_rotate_until = 0.0
        self.align_locked_settle_until = 0.0
    ...
    def _init_yolo(self):
        try:
            ...
            rospy.loginfo(
                "line_cy_task waiting for YOLO model load and warmup: %s",
                self.yolo_detector.model_path,
            )
            self.publish(0, 0)
            # Load once so route-time inference does not pay a model load cost.
            ...
            self.yolo_ready = False
            return
        self.yolo_camera = CameraReader(self.yolo_camera_index)
        ...
        rospy.loginfo(
            "line_cy_task YOLO model loaded and warmup complete, "
            "enabled camera=%d backend=%s model=%s interval=%d imgsz=%d "
            "conf=%.2f nms=%.2f stop=%s debug=%s",
            ...
            self.yolo_nms_threshold, self.yolo_stop_enabled,
    ...
    def _run_timed_turn_phase(self, now):
        ...
        if next_phase is not None:
            ...
            final_turn_completed = (
                next_phase == "EXIT_STRAIGHT"
                and self.task_index == len(TASK_TURN_COMMANDS) - 1
                and self.turn_cmd == "right"
            )
            if final_turn_completed:
                rospy.loginfo(
                    "line_cy_task final right turn completed, exit for %.2f seconds",
    ...
    def _reset_entry_alignment_lock(self):
        self.align_locked_angle = None
        self.align_locked_angular = 0.0
        self.align_locked_rotate_until = 0.0
        self.align_locked_settle_until = 0.0

    def _current_entry_bar_angle(self):
        cross = getattr(self, "last_crosswalk", None)
        if cross is None:
            return None
        if getattr(cross, "candidate", False) and cross.stop_angle is not None:
            return cross.stop_angle
        return cross.tracking_angle

    def _lock_entry_alignment(self, now=None):
        angle = self._current_entry_bar_angle()
        if angle is None:
            self._reset_entry_alignment_lock()
            return False
        now = rospy.get_time() if now is None else float(now)
        plan = locked_entry_alignment_plan(angle, now)
        self.align_locked_angle = plan["angle"]
        self.align_locked_angular = plan["angular"]
        self.align_locked_rotate_until = plan["rotate_until"]
        self.align_locked_settle_until = plan["settle_until"]
        rospy.loginfo(
            "line_cy_task locked entry align angle=%.1f angular=%.2f "
            "rotate=%.2fs settle=%.2fs",
            self.align_locked_angle,
            self.align_locked_angular,
            max(0.0, self.align_locked_rotate_until - now),
            max(0.0, self.align_locked_settle_until
                - self.align_locked_rotate_until),
        )
        return True

    def _run_locked_entry_alignment(self, now):
        if self.align_locked_angle is None:
            return False, None
        now = float(now)
        if now < self.align_locked_rotate_until:
            self.publish(0, self.align_locked_angular)
            return True, None
        if now < self.align_locked_settle_until:
            self.publish(0, 0)
            return True, None
        self.publish(0, 0)
    ...
    def _yolo_target_candidates(self, detections):
        return [
    ...
    def _yolo_event_type(self, detection):
        if detection is None:
            return None
        if detection.class_name in ("General population", "Medical population"):
            return "people"
        if detection.class_name in dict(YOLO_BUILDING_MESSAGES):
            return "building"
        return None

    def _select_yolo_stop_event(self, detections, now):
        candidates = self._yolo_target_candidates(detections)
        buildings = [
            item for item in candidates
            if self._yolo_event_type(item) == "building"
        ]
        if buildings and self.yolo_building_stop_count < self.yolo_building_stop_limit:
            return "building", max(buildings, key=lambda item: item.confidence)

        people = [
            item for item in candidates
            if self._yolo_event_type(item) == "people"
        ]
        if not people or self.yolo_people_stop_count >= self.yolo_people_stop_limit:
            return None, None
        if (self.yolo_last_people_stop_time is not None
                and float(now) - self.yolo_last_people_stop_time
                < self.yolo_people_stop_cooldown):
            return None, None
        return "people", max(people, key=lambda item: item.confidence)

    def _record_yolo_stop_event(self, event_type, detection, now):
        self.yolo_stop_event_type = event_type
        self.yolo_stop_detection = detection
        if event_type == "building":
            self.yolo_building_stop_count += 1
        elif event_type == "people":
            self.yolo_people_stop_count += 1
            self.yolo_last_people_stop_time = float(now)

    def _reset_yolo_stop_limits(self):
        self.yolo_building_stop_count = 0
        self.yolo_people_stop_count = 0
        self.yolo_last_people_stop_time = None
        self.yolo_stop_event_type = None
    ...
    def _selected_yolo_target(self, detections):
        event_type, detection = self._select_yolo_stop_event(
            detections, rospy.get_time()
        )
        return detection

    def _log_yolo_skip(self, detections, event_type, selected):
        if selected is not None:
            return
        interesting = [
            item for item in detections
            if item.confidence >= max(0.20, self.yolo_confidence * 0.5)
        ]
        if not interesting:
            return
        now = rospy.get_time()
        last_log = getattr(self, "yolo_last_skip_log_time", None)
        if last_log is not None and now - last_log < 1.0:
            return
        self.yolo_last_skip_log_time = now
        summary = []
        for item in sorted(interesting, key=lambda value: value.confidence,
                           reverse=True)[:4]:
            summary.append("{} {:.2f} target={} center={}".format(
                item.class_name, item.confidence, item.target, item.in_center
            ))
        rospy.loginfo(
            "line_cy_task YOLO no stop: event=%s building=%d/%d people=%d/%d "
            "people_cooldown=%.1f seen=[%s]",
            event_type,
            self.yolo_building_stop_count, self.yolo_building_stop_limit,
            self.yolo_people_stop_count, self.yolo_people_stop_limit,
            self.yolo_people_stop_cooldown,
            "; ".join(summary),
        )

    def _log_yolo_stop_summary(self, detections):
        medical_count = sum(
            1 for item in detections
            if item.class_name == "Medical population"
            and item.confidence >= self.yolo_confidence
        )
        general_count = sum(
            1 for item in detections
            if item.class_name == "General population"
            and item.confidence >= self.yolo_confidence
        )
        if medical_count or general_count:
            rospy.loginfo(
                "检测到人群：医疗人群%d个，普通人群%d个",
                medical_count, general_count,
            )
        detected_classes = set(
            item.class_name for item in detections
            if item.confidence >= self.yolo_confidence
        )
        for class_name, message in YOLO_BUILDING_MESSAGES:
    ...
    def _maybe_enter_yolo_stop(self, observation):
        ...
        if not sampled:
            return False
        now = rospy.get_time()
        event_type, selected = self._select_yolo_stop_event(detections, now)
        if selected is None or not self.yolo_stop_enabled:
            self._log_yolo_skip(detections, event_type, selected)
            return False
        self._record_yolo_stop_event(event_type, selected, now)
        self.yolo_stop_reported = False
    ...
    def _handle_yolo_stop(self, now):
        ...
        if not self.yolo_stop_reported:
            ...
            if sampled and self.yolo_read_seq > self.yolo_stop_report_seq:
                self._log_yolo_stop_summary(detections)
    ...
    def process(self, raw_frame):
        ...
        if self.state == "FOLLOW":
            ...
            if self.state == "FOLLOW" and self._maybe_enter_yolo_stop(observation):
                pass
    ...
    def draw_yolo_debug(self):
        ...
        height, width = frame.shape[:2]
        ratio = clamp(getattr(self, "yolo_center_band_ratio",
                              YOLO_CENTER_BAND_RATIO), 0.0, 1.0)
        left = int(round(width * (1.0 - ratio) * 0.5))
        right = int(round(width - left))
        cv2.line(frame, (left, 0), (left, height - 1), (255, 255, 0), 1)
        cv2.line(frame, (right, 0), (right, height - 1), (255, 255, 0), 1)
        for item in detections:
            ...
            cv2.circle(frame, (int(round(item.center_x)), int(round(item.center_y))),
                       3, color, -1)
    ...
    def draw_debug(self, frame):
        ...
        locked_angle = getattr(self, "align_locked_angle", None)
        lock_text = "--" if locked_angle is None else "{:.1f}".format(locked_angle)
