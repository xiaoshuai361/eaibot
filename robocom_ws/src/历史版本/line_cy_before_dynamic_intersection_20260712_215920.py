...
SINGLE_CENTER_FACTOR = 0.8   # 单边补线中心距离，按车道宽度倍数
...
STOP_MAX_ANGLE_DEG = 45.0            # 停车横条最大倾角；竖直边线会被排除
...
class LaneFollower:
    ...
    def locked_maneuver_mode(self, cmd, elapsed, binary=None, locked_side=None):
        if elapsed < self.enter_intersection_straight_time:
            return "normal"
        side = locked_side or self.locked_maneuver_side(cmd, binary)
        if side == "right":
            return "right_only"
        return "left_only"
        ...
        while not rospy.is_shutdown() and rospy.get_time() - start <= self.intersection_max_time:
            ...
            if self.maneuver_strict_side_follow:
                if elapsed >= self.enter_intersection_straight_time and maneuver_locked_side is None:
                    maneuver_locked_side = self.locked_maneuver_side(cmd, lane_binary)
                    rospy.loginfo("路口锁定单边补线: side=%s", maneuver_locked_side)
            ...
            locked_debug = self.last_debug.copy()
            recover_kalman = self._make_kalman()
            recover_kalman.statePost = np.array([[self.last_mid], [0]], np.float32)
            _, _, _, recover_debug = self.vision.scan(
                lane_binary,
                recover_kalman,
                self.last_mid,
                0,
                self.lane_width(frame.shape[1]),
                "normal",
                None,
            )
            self.last_debug = locked_debug
            ...
            can_recover = (
                ...
                and recover_debug.get("dual_rows", 0) >= 2
