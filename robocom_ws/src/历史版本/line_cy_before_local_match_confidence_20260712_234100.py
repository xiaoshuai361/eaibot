...
# 常用调车参数：ROS 参数名是小写常量名，例如 LINEAR_SPEED -> _line_speed:=0.10。
# 详细调参步骤见 /home/eaibot/zcy/循迹操作.md。

# 摄像头 / 安全开关
LANE_CAM_INDEX = 2        # 巡线摄像头编号
PROCESS_WIDTH = 640      # 识别宽度；固定 640 便于调 PID
DRY_RUN = True           # true 时不控车
DETECT_ONLY = True       # true 时只识别不走状态机
DEBUG_VIEW = True        # 显示处理图
RAW_VIEW = False         # 显示原图

# 速度 / 转向
LINEAR_SPEED = 0.10              # 普通巡线速度
SINGLE_LINE_SPEED = 0.08         # 单边线/补线速度
MAX_ANGULAR = 0.6                # 最大角速度；拐不过加，抖就降
SINGLE_LINE_MIN_ANGULAR = 0.15   # 单边转弯最小角速度
ANGULAR_SMOOTH_KEEP = 0.70       # 转向平滑；大=稳，小=灵敏
ANGULAR_STEP_LIMIT = 0.05        # 单帧转向变化上限

# PID
PID_KP_SMALL = 0.0028    # 小偏差比例；直线摆动先降
PID_KD_SMALL = 0.0008    # 小偏差阻尼
PID_KP_BIG = 0.03        # 大偏差比例；弯道拐不过加
PID_KD_BIG = 0.002       # 大偏差阻尼
PID_KI = 0               # 积分；通常不用
DEV_THRESHOLD = 100      # 大/小偏差分界，像素
PID_BLEND_WIDTH = 80     # 大小偏差增益过渡半宽，避免弯道阈值处转向突跳

# ROI / 黑线
ROI_TOP_RATIO = 0.24         # 绿色 ROI 上边界
ROI_BOTTOM_RATIO = 0.76      # 绿色 ROI 下边界
SINGLE_CENTER_FACTOR = 0.65   # 单边补线中心距离，按车道宽度倍数
BLACK_V_MAX = 80             # 黑色亮度上限；黑线断加，噪声多降
ADAPTIVE_BLOCK_SIZE = 31     # 自适应阈值块大小
ADAPTIVE_C = 5               # 自适应阈值偏置
BLUR_KERNEL_SIZE = 5         # 模糊核
MORPH_KERNEL_SIZE = 3        # 形态学核

# 斑马线停车
STOP_CONFIDENCE_MIN = 0.68           # 入口触发置信度
STOP_FRONT_CENTER_MARGIN_RATIO = 0.28 # 横条偏离画面中心容忍
STOP_MAX_ANGLE_DEG = 35.0            # 停车横条最大倾角；竖直边线会被排除
STOP_STABLE_FRAMES = 3               # 连续确认帧数
STOP_HOLD_TIME = 1.0                 # 摆正后停车时间
STOP_COOLDOWN_TIME = 5.0             # 通过后防重复停车时间

# 靠近 / 摆正
APPROACH_CROSSWALK_SPEED = 0.06  # 靠近横条速度
ALIGN_TRIGGER_Y_RATIO = 0.82     # 横条到此高度开始摆正
ALIGN_KP = 0.025                 # 摆正比例
ALIGN_MAX_ANGULAR = 0.35         # 摆正最大角速度
ALIGN_MIN_ANGULAR = 0.08         # 摆正最小角速度
ALIGN_ANGLE_TOLERANCE_DEG = 3.0  # 认为水平的角度误差
ALIGN_STABLE_FRAMES = 5          # 水平稳定帧数
ALIGN_TIMEOUT = 5.0              # 摆正超时
ALIGN_ANGULAR_SIGN = 1.0         # 摆反时改 -1.0
CROSSWALK_LOST_FRAMES = 6        # 横条允许丢失帧数

# 路口通过
SIDE_FOLLOW_SPEED = 0.10             # 路口单边补线速度
ENTER_INTERSECTION_STRAIGHT_TIME = 0.6 # 离开横条后先直走时间
INTERSECTION_MIN_TIME = 1.2          # 路口最短时间
INTERSECTION_MAX_TIME = 15.0         # 路口最长保护时间
RECOVER_DUAL_FRAMES = 10             # 双线恢复确认帧数
CROSSWALK_CLEAR_CONFIDENCE = 0.45    # 斑马线清除置信度
CROSSWALK_TRACK_CONFIDENCE = 0.52    # 接近/摆正跟踪置信度
CROSSWALK_CLEAR_FRAMES = 8           # 斑马线消失确认帧数
EXIT_CROSSWALK_FRAMES = 3            # 出口横线连续确认帧数；只恢复巡线，不停车
EXIT_CROSSWALK_Y_RATIO = 0.68        # 出口横线到达近端后才恢复，避免远处斑马纹提前触发
MANEUVER_CROSSWALK_MEMORY_FRAMES = 16 # 通过时斑马线屏蔽记忆
LEFT_TURN_BIAS = 0.12                # 左转偏置；不够加，过头降
RIGHT_TURN_BIAS = 0.12               # 右转偏置；不够加，过头降
STRAIGHT_BIAS = 0.0                  # 直行偏置；偏左/右微调
MANEUVER_STRICT_SIDE_FOLLOW = True   # 路口内只看锁定单边线
MANEUVER_STRICT_BAND_RATIO = 0.14    # 锁边屏蔽带宽度
MANEUVER_LOCK_CONFIRM_FRAMES = 3     # 同一侧连续确认后才锁定
MANEUVER_STRAIGHT_FOLLOW_SIDE = "auto" # straight 可填 auto/left/right
...
# 补线距离 / 车道宽度
LANE_WIDTH_PIXELS = 0.0          # 固定补线宽度；0=自动，填处理图 width
DEFAULT_LANE_WIDTH_RATIO = 0.56  # 初始车道宽度占画面比例
LANE_WIDTH_MIN_RATIO = 0.35      # 自动宽度下限
LANE_WIDTH_MAX_RATIO = 0.85      # 自动宽度上限
PAIR_WIDTH_TOLERANCE = 0.50      # 双线配对宽度容忍
PAIR_MAX_GAP_RATIO = 0.96        # 双线最大间距
PAIR_CENTER_JUMP_RATIO = 0.30    # 双线中心跳变容忍
OUTSIDE_MARGIN_RATIO = 0.10      # 忽略左右外侧线余量
SINGLE_WIDTH_FLOOR_RATIO = 0.52  # 单边补线最小宽度
LANE_CONTINUITY_JUMP_RATIO = 0.12 # 边线连续性阈值
SINGLE_SIDE_HINT_FRAMES = 12     # 单边方向记忆帧数
SINGLE_TURN_CONFIRM_FRAMES = 4   # 单边转向确认帧数
SINGLE_TURN_RELEASE_FRAMES = 6   # 单边转向释放帧数
KALMAN_FAIL_MAX = 8              # 卡尔曼连续失败重置帧数
LANE_LOCK_MASK_RADIUS_RATIO = 0.035 # 上帧边线锁定宽度；防边线被误判横条
LANE_LOCK_STOP_OVERLAP = 0.18     # 横条与锁定边线重合超过此比例就拒绝
LANE_LOCK_MIN_RUN_RATIO = 0.45    # 还需沿横条连续重合足够长；正常边线交点不会触发
...
def blended_pid_gains(deviation, kp_small, kd_small, kp_big, kd_big, threshold, blend_width):
    width = max(1.0, float(blend_width))
    ratio = clamp((abs(float(deviation)) - float(threshold) + width) / (2.0 * width), 0.0, 1.0)
    return (
        float(kp_small) + (float(kp_big) - float(kp_small)) * ratio,
        float(kd_small) + (float(kd_big) - float(kd_small)) * ratio,
    )


def confirm_maneuver_side(pending_side, hits, locked_side, detected_side, required_frames):
    if locked_side in ("left", "right"):
        return pending_side, hits, locked_side
    if detected_side not in ("left", "right"):
        return None, 0, None
    hits = hits + 1 if detected_side == pending_side else 1
    pending_side = detected_side
    if hits >= max(1, int(required_frames)):
...
class LineVision:
    ...
    def __init__(self):
        ...
        self.stop_max_angle_deg = STOP_MAX_ANGLE_DEG
    ...
    def locked_side_reference(self, groups, width, follow_mode):
        if not groups:
            return None
        if follow_mode == "left_only":
            candidates = [g for g in groups if g[2] <= width * 0.72] or groups
            return min(candidates, key=lambda g: g[2])
        if follow_mode == "right_only":
            candidates = [g for g in groups if g[2] >= width * 0.28] or groups
            return max(candidates, key=lambda g: g[2])
        return None
    ...
    def row_center(self, groups, width, expected_width, prior_center, follow_mode, side_hint):
        if follow_mode in ("left_only", "right_only"):
            ref = self.locked_side_reference(groups, width, follow_mode)
            ignored = [group for group in groups if group != ref]
            if ref is None:
                return None, [], ignored, "missing", None, None, None, None
            if follow_mode == "left_only":
    ...
    def _near_frame_side(self, entries, key, width, side):
        values = [float(item[key]) for item in entries if item.get(key) is not None]
        if len(values) < 2:
            return False
        median = float(np.median(values))
        if side == "left":
            return median <= width * 0.16 and min(values) <= width * 0.10
        return median >= width * 0.84 and max(values) >= width * 0.90
    def _fix_discontinuous_pairs(self, candidates, entries, width, lane_width):
        ...
        if left_ok == right_ok:
            return candidates, entries
        if left_ok and self._near_frame_side(pair_entries, "right_edge", width, "right"):
            return candidates, entries
        if right_ok and self._near_frame_side(pair_entries, "left_edge", width, "left"):
            return candidates, entries
        fixed_entries = []
        fixed_candidates = []
        for item in entries:
    ...
    def detect_stopline_before_crosswalk(self, binary, lane_lock_mask=None):
        height, width = binary.shape[:2]
        roi = binary.copy()
        ...
        for contour in contours:
            ...
            stop_angle_ok = abs(candidate["angle_deg"]) <= self.stop_max_angle_deg
            hull = cv2.convexHull(contour)
            solidity = area / float(cv2.contourArea(hull) + 1.0)
            ...
            straight_stop = rect_fill > 0.55 and solidity > 0.70
            overlap, run_ratio = self._lane_lock_overlap(contour, polygon, lane_lock_mask)
            locked_edge = overlap >= LANE_LOCK_STOP_OVERLAP and run_ratio >= LANE_LOCK_MIN_RUN_RATIO
            if stop_angle_ok and straight_stop and not locked_edge and (axis_stop or rotated_stop) and cy > height * 0.22:
        ...
        stops.extend(self._connected_stopline_candidates(binary, stripes, lane_lock_mask))
        best = None
        for stop in stops:
            ...
            if scored is not None and (best is None or scored["confidence"] > best["confidence"]):
                best = scored
        if best is None:
            # 没有完整斑马线入口时，单根横条只作为接近/摆正阶段的跟踪目标。
            # 不放进 stop_polygon，避免正常巡线时把误识别的边线遮罩掉。
            selected = self._select_crosswalk_stripes(stripes, binary.shape)
            confidence = min(0.6, len(selected) / 5.0 * 0.45)
            tracking_stop = self._select_tracking_stop(stops, binary.shape)
            tracking_confidence = (
                0.0 if tracking_stop is None else self._tracking_stop_confidence(tracking_stop, binary.shape)
            )
            return {
                "candidate": False,
                ...
                "tracking_stop_box": None if tracking_stop is None else tracking_stop["box"],
                "tracking_stop_polygon": None if tracking_stop is None else tracking_stop["polygon"],
                "tracking_stop_angle_deg": None if tracking_stop is None else tracking_stop["angle_deg"],
                "tracking_stop_bottom_y": 0 if tracking_stop is None else tracking_stop["bottom_y"],
        ...
        if not best["candidate"]:
            tracking_confidence = self._tracking_stop_confidence(
                {
                    "bottom_y": best["stop_bottom_y"],
                    "long_side": max(best["stop_box"][2], best["stop_box"][3]),
                    "angle_deg": best["stop_angle_deg"],
                },
                binary.shape,
            )
            best.update({
                "tracking_confidence": tracking_confidence,
                "tracking_stop_box": best["stop_box"],
                "tracking_stop_polygon": best["stop_polygon"],
                "tracking_stop_angle_deg": best["stop_angle_deg"],
                "tracking_stop_bottom_y": best["stop_bottom_y"],
                "stop_box": None,
                "stop_polygon": None,
                "stop_angle_deg": None,
                "stop_bottom_y": 0,
        ...
        return best
    def _connected_stopline_candidates(self, binary, stripes, lane_lock_mask):
        """从斑马纹下方与边线粘连的轮廓中补提停车横线。"""
        height, width = binary.shape[:2]
        selected = self._select_crosswalk_stripes(stripes, binary.shape)
        if len(selected) < 3:
            selected = self._select_early_crosswalk_stripes(stripes, binary.shape)
        if not selected:
            return []
        stripe_bottom = max(item["bottom_y"] for item in selected)
        lines = cv2.HoughLinesP(
            binary,
            1,
            np.pi / 180.0,
            threshold=max(35, int(width * 0.08)),
            minLineLength=max(60, int(width * 0.24)),
            maxLineGap=max(12, int(width * 0.04)),
        )
        if lines is None:
            return []
        ...
        for x1, y1, x2, y2 in lines[:, 0, :]:
            ...
            while angle < -90.0:
                angle += 180.0
            if abs(angle) > self.stop_max_angle_deg:
                continue
            center_x = (x1 + x2) * 0.5
            center_y = (y1 + y2) * 0.5
            if not (stripe_bottom - height * 0.01 <= center_y <= stripe_bottom + height * 0.35):
                continue
            thickness = max(8.0, width * 0.025)
            rect = ((float(center_x), float(center_y)), (length, thickness), angle)
            polygon = np.rint(cv2.boxPoints(rect)).astype(np.int32)
            contour = polygon.reshape(-1, 1, 2)
            overlap, run_ratio = self._lane_lock_overlap(contour, polygon, lane_lock_mask)
            if overlap >= LANE_LOCK_STOP_OVERLAP and run_ratio >= LANE_LOCK_MIN_RUN_RATIO:
                continue
            x, y, w, h = cv2.boundingRect(contour)
            candidate = {
                "box": (int(x), int(y), int(w), int(h)),
                ...
                "long_side": length,
                "short_side": thickness,
                "ratio": length / thickness,
            }
            candidates.append(candidate)
        return candidates
    def _select_tracking_stop(self, stops, shape):
        """对齐阶段只跟踪正前方停车横条；不把它当作路口入口触发。"""
        height, width = shape[:2]
        best = None
        for stop in stops:
            if not self._stop_is_in_front(stop["box"], shape):
                continue
            if abs(stop["angle_deg"]) > self.stop_max_angle_deg:
                continue
            if stop["bottom_y"] < height * 0.35:
                continue
            center_penalty = abs(stop["center"][0] - width * 0.5) / float(width)
            bottom_score = stop["bottom_y"] / float(height)
            length_score = min(1.0, stop["long_side"] / max(width * 0.45, 1.0))
            angle_score = max(0.0, 1.0 - abs(stop["angle_deg"]) / max(self.stop_max_angle_deg, 1.0))
            score = bottom_score * 0.45 + length_score * 0.35 + angle_score * 0.20 - center_penalty * 0.30
            if best is None or score > best[0]:
                best = (score, stop)
        return None if best is None else best[1]
    def _tracking_stop_confidence(self, stop, shape):
        height, width = shape[:2]
        bottom_score = clamp(stop["bottom_y"] / float(max(height, 1)), 0.0, 1.0)
        length_score = clamp(stop["long_side"] / float(max(width * 0.45, 1.0)), 0.0, 1.0)
        angle_score = max(0.0, 1.0 - abs(stop["angle_deg"]) / max(self.stop_max_angle_deg, 1.0))
        return 0.52 + 0.18 * bottom_score + 0.15 * length_score + 0.10 * angle_score
    ...
    def _select_early_crosswalk_stripes(self, stripes, shape):
        """首批只露出一两条时保留矩形条纹，最终仍须与下方横条严格配对。"""
        height, width = shape[:2]
        selected = [
            item for item in stripes
            if width * 0.08 <= item["center"][0] <= width * 0.92
        ]
        if not 1 <= len(selected) <= 2:
            return []
        if len(selected) == 2:
            first, second = selected
            same_band = abs(first["center"][1] - second["center"][1]) <= height * 0.18
            angle_ok = undirected_angle_delta_deg(first["angle_deg"], second["angle_deg"]) <= 20.0
            long_ratio = max(first["long_side"], second["long_side"]) / max(
                min(first["long_side"], second["long_side"]), 1.0
            )
            short_ratio = max(first["short_side"], second["short_side"]) / max(
                min(first["short_side"], second["short_side"]), 1.0
            )
            if not (same_band and angle_ok and long_ratio <= 1.8 and short_ratio <= 1.8):
                return []
    ...
    def _stop_is_in_front(self, stop_box, shape):
        ...
        overlaps_center_band = x <= center_x + margin and x + w >= center_x - margin
        return crosses_center or (wide_enough and (center_in_band or overlaps_center_band))
    def _score_stop_group(self, stop, stripes, shape):
        height, width = shape[:2]
        sx, sy, sw, sh = stop["box"]
        selected = self._select_crosswalk_stripes(stripes, shape)
        if len(selected) < 3:
            selected = self._early_stripes_matching_stop(stop, stripes, shape)
            if not selected:
                return None
            return {
                "confidence": max(self.stop_confidence_min, 0.72),
                "order_ok": True,
                "stop_box": stop["box"],
        ...
        matched = self._stripes_matching_stop(stop, selected, shape)
        ...
        order_ok = bool(matched)
        if stop_y is not None and not order_ok:
    ...
    def _early_stripes_matching_stop(self, stop, stripes, shape):
        """用位置、覆盖范围和近垂直方向确认首个条纹确实属于停车横条。"""
        selected = self._select_early_crosswalk_stripes(stripes, shape)
        return self._stripes_matching_stop(stop, selected, shape)
    def _stripes_matching_stop(self, stop, stripes, shape):
        """返回横条实际接住的局部条纹，不要求横条跨过整组平均中心。"""
        height, width = shape[:2]
        if not stripes or stop["long_side"] < width * 0.25:
            return []
        if abs(stop["angle_deg"]) > self.stop_max_angle_deg:
            return []
        sx, sy, sw, sh = stop["box"]
        ...
        for stripe in stripes:
            stripe_x = stripe["center"][0]
            if not (sx - width * 0.03 <= stripe_x <= sx + sw + width * 0.03):
                continue
            stop_y = self._line_y_at_x(stop, stripe_x)
            if stop_y is None:
                continue
            below_stripe = (
                stripe["bottom_y"] - height * 0.01
                <= stop_y
                <= stripe["bottom_y"] + height * 0.35
            )
            perpendicular = undirected_angle_delta_deg(
                stripe["angle_deg"], stop["angle_deg"]
            ) >= 65.0
            if below_stripe and perpendicular:
    ...
    def _lane_lock_overlap(self, contour, polygon, lane_lock_mask):
        if lane_lock_mask is None:
            return 0.0, 0.0
        contour_mask = np.zeros_like(lane_lock_mask)
        cv2.drawContours(contour_mask, [contour], -1, 255, thickness=cv2.FILLED)
        area = float(np.count_nonzero(contour_mask))
        if area <= 0.0:
            return 0.0, 0.0
        overlap_mask = cv2.bitwise_and(contour_mask, lane_lock_mask)
        overlap_y, overlap_x = np.where(overlap_mask > 0)
        if len(overlap_x) == 0:
            return 0.0, 0.0
        ...
        long_side = float(np.linalg.norm(edge))
        if long_side < 1.0:
            return len(overlap_x) / area, 0.0
        direction = edge / long_side
        projections = overlap_x * direction[0] + overlap_y * direction[1]
        occupied = np.zeros(max(1, int(np.ceil(long_side)) + 1), dtype=np.uint8)
        indices = np.rint(projections - projections.min()).astype(np.int32)
        occupied[np.clip(indices, 0, len(occupied) - 1)] = 1
        runs = np.diff(np.flatnonzero(np.r_[1, occupied[1:] != occupied[:-1], 1]))
        values = occupied[np.flatnonzero(np.r_[1, occupied[1:] != occupied[:-1]])]
        longest_run = max([run for run, value in zip(runs, values) if value] or [0])
        return len(overlap_x) / area, longest_run / long_side
...
class LaneFollower:
    def __init__(self):
        ...
        params = (
            ...
            ("exit_crosswalk_frames", EXIT_CROSSWALK_FRAMES, int),
            ("exit_crosswalk_y_ratio", EXIT_CROSSWALK_Y_RATIO, float),
            ("left_turn_bias", LEFT_TURN_BIAS, float),
            ("right_turn_bias", RIGHT_TURN_BIAS, float),
            ("straight_bias", STRAIGHT_BIAS, float),
            ("maneuver_strict_side_follow", MANEUVER_STRICT_SIDE_FOLLOW, bool),
            ("maneuver_strict_band_ratio", MANEUVER_STRICT_BAND_RATIO, float),
            ("maneuver_lock_confirm_frames", MANEUVER_LOCK_CONFIRM_FRAMES, int),
            ("maneuver_straight_follow_side", MANEUVER_STRAIGHT_FOLLOW_SIDE, str),
    ...
    def configure_vision_params(self):
        ...
        self.vision.stop_max_angle_deg = float(rospy.get_param("~stop_max_angle_deg", STOP_MAX_ANGLE_DEG))
    ...
    def update_pid_gain(self, deviation):
        self.pid.kp, self.pid.kd = blended_pid_gains(
            deviation,
            rospy.get_param("~kp_small", PID_KP_SMALL),
            rospy.get_param("~kd_small", PID_KD_SMALL),
            rospy.get_param("~kp_big", PID_KP_BIG),
            rospy.get_param("~kd_big", PID_KD_BIG),
            DEV_THRESHOLD,
            PID_BLEND_WIDTH,
        )
    ...
    def line_control(self, frame, binary, follow_mode="normal", speed=None, bias=0.0, allow_single_turn=True):
        ...
        self.update_lane_lock_mask(binary, debug)
        ...
        if self.failed_count > 3:
            ...
            if follow_mode in ("left", "left_only"):
                angular = max(angular, self.left_turn_bias)
            elif follow_mode in ("right", "right_only"):
    ...
    def update_lane_lock_mask(self, binary, debug):
        height, width = binary.shape[:2]
        points = []
        for row in debug.get("lane_rows", []):
            y = int(row["y"])
            for key in ("left_x", "right_x"):
                if row.get(key) is not None:
                    points.append((int(row[key]), y))
        if len(points) < 2:
            self.lane_lock_mask = None
            return
        mask = np.zeros_like(binary)
        radius = max(6, int(width * LANE_LOCK_MASK_RADIUS_RATIO))
        for x, y in points:
            cv2.circle(mask, (int(clamp(x, 0, width - 1)), int(clamp(y, 0, height - 1))), radius, 255, -1)
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (radius * 2 + 1, radius * 2 + 1))
        self.lane_lock_mask = cv2.dilate(mask, kernel)
    ...
    def visible_maneuver_side(self, binary):
        height, width = binary.shape[:2]
        top = int(height * self.vision.roi_top_ratio)
        bot = int(height * self.vision.roi_bottom_ratio)
        rows = np.linspace(bot, top, self.vision.scan_rows).astype(int)
        left_score = right_score = 0.0
        for index, y in enumerate(rows):
            groups = self.vision.row_groups(np.where(binary[y, :] == 255)[0], width)
            weight = 1.0 + (len(rows) - index) * 0.25
            if any(group[2] <= width * 0.48 for group in groups):
                left_score += weight
            if any(group[2] >= width * 0.52 for group in groups):
                right_score += weight
        return "right" if right_score > left_score else "left"
    def _trace_maneuver_side(self, binary, side, prior_points=None):
        height, width = binary.shape[:2]
        rows = np.linspace(
            int(height * self.vision.roi_bottom_ratio),
            int(height * self.vision.roi_top_ratio),
            self.vision.scan_rows,
        ).astype(int)
        prior_by_y = {} if not prior_points else {int(y): float(x) for x, y in prior_points}
        points = []
        last_x = None
        max_jump = width * max(0.08, float(self.maneuver_strict_band_ratio))
        for y in rows:
            groups = self.vision.row_groups(np.where(binary[y, :] == 255)[0], width)
            groups = [g for g in groups if (g[2] <= width * 0.58 if side == "left" else g[2] >= width * 0.42)]
            if not groups:
                continue
            expected = prior_by_y.get(int(y), last_x)
            if expected is None:
                group = min(groups, key=lambda g: g[2]) if side == "left" else max(groups, key=lambda g: g[2])
            else:
                group = min(groups, key=lambda g: abs(g[2] - expected))
                if abs(group[2] - expected) > max_jump:
                    continue
            last_x = float(group[2])
            points.append((int(round(last_x)), int(y)))
        return points
    def acquire_maneuver_track(self, binary):
        """选择扫描行中连续存在更多的一侧；选定后本次路口不再换边。"""
        left = self._trace_maneuver_side(binary, "left")
        right = self._trace_maneuver_side(binary, "right")
        strong = max(len(left), len(right))
        weak = min(len(left), len(right))
        if strong < 3 or weak >= 3 or strong < weak + 2:
            return None, []
        return ("right", right) if len(right) > len(left) else ("left", left)
    def filter_maneuver_track(self, binary, side, prior_points):
        """只保留上一帧轨迹附近的边线，屏蔽路口内其他所有黑色区域。"""
        points = self._trace_maneuver_side(binary, side, prior_points)
        if len(points) < 2:
            return np.zeros_like(binary), prior_points
        height, width = binary.shape[:2]
        radius = max(8, int(width * float(self.maneuver_strict_band_ratio) * 0.5))
        mask = np.zeros_like(binary)
        ordered = sorted(points, key=lambda item: item[1])
        cv2.polylines(mask, [np.asarray(ordered, dtype=np.int32).reshape(-1, 1, 2)], False, 255, radius * 2)
        return cv2.bitwise_and(binary, mask), points
    def update_exit_crosswalk_hits(self, entry_cleared, hits, visible):
        if not entry_cleared:
            return 0
        return hits + 1 if visible else max(0, hits - 1)
    def maneuver_exit_visible(self, stop_result, frame_height):
        if frame_height <= 0:
            return False
        bottom = stop_result.get("stop_bottom_y", 0)
        visible = bool(stop_result.get("candidate", False))
        if not visible:
            bottom = stop_result.get("tracking_stop_bottom_y", 0)
            visible = stop_result.get("tracking_confidence", 0.0) >= self.crosswalk_track_confidence
        return visible and bottom / float(frame_height) >= self.exit_crosswalk_y_ratio
    def locked_maneuver_side(self, cmd, binary):
        if cmd in ("left", "right"):
            return cmd
        side = str(self.maneuver_straight_follow_side).lower().strip()
        if side in ("left", "right"):
            return side
        if binary is None:
            return "left"
        return self.visible_maneuver_side(binary)
    def locked_maneuver_mode(self, cmd, elapsed, binary=None, locked_side=None):
        if elapsed < self.enter_intersection_straight_time or locked_side not in ("left", "right"):
            return "normal"
    ...
    def crosswalk_mask_result(self, current, remembered=None):
        ...
        return merged
    def tracking_stop_result(self, stop_result):
        if not stop_result or stop_result.get("tracking_stop_polygon") is None:
            return None
        return {
            "candidate": False,
            "confidence": stop_result.get("tracking_confidence", 0.0),
            "stop_box": stop_result.get("tracking_stop_box"),
            "stop_polygon": stop_result.get("tracking_stop_polygon"),
            "stop_angle_deg": stop_result.get("tracking_stop_angle_deg"),
            "stop_bottom_y": stop_result.get("tracking_stop_bottom_y", 0),
            "stripe_polygons": [],
            "loose_stripe_polygons": [],
        }
    def run_maneuver(self, cmd):
        ...
        crosswalk_clear = 0
        entry_cleared = False
        exit_crosswalk_hits = 0
        crosswalk_memory = None
        crosswalk_memory_frames = 0
        maneuver_locked_side = None
        maneuver_track = []
        pending_side = None
        pending_hits = 0
        ...
        while not rospy.is_shutdown() and rospy.get_time() - start <= self.intersection_max_time:
            ...
            self.last_stop = self.vision.detect_stopline_before_crosswalk(binary, self.lane_lock_mask)
            ...
            if self.maneuver_strict_side_follow:
                if elapsed >= self.enter_intersection_straight_time and maneuver_locked_side is None:
                    detected_side, detected_track = self.acquire_maneuver_track(lane_binary)
                    pending_side, pending_hits, maneuver_locked_side = confirm_maneuver_side(
                        pending_side,
                        pending_hits,
                        maneuver_locked_side,
                        detected_side,
                        self.maneuver_lock_confirm_frames,
                    )
                    if detected_side == pending_side and detected_track:
                        maneuver_track = detected_track
                    if maneuver_locked_side is not None and pending_hits == self.maneuver_lock_confirm_frames:
                        self.pid.reset()
                        self.failed_count = 0
                        rospy.loginfo("路口锁定连续边线: side=%s", maneuver_locked_side)
                if maneuver_locked_side is not None:
                    lane_binary, maneuver_track = self.filter_maneuver_track(
                        lane_binary, maneuver_locked_side, maneuver_track
                    )
                active_mode = self.locked_maneuver_mode(cmd, elapsed, lane_binary, maneuver_locked_side)
                active_bias = 0.0 if active_mode == "normal" else bias
                allow_single_turn = False
            else:
                active_mode, active_bias, allow_single_turn = self.maneuver_follow_choice(cmd, elapsed, mode, bias)
            waiting_for_lock = (
                self.maneuver_strict_side_follow
                and elapsed >= self.enter_intersection_straight_time
                and maneuver_locked_side is None
            )
            if waiting_for_lock:
                self.last_angular = clamp(self.last_angular * 0.8, -self.search_angular_limit, self.search_angular_limit)
                self.publish_cmd(min(self.side_follow_speed, self.search_speed), self.last_angular)
                centers = []
                self.last_debug = {"maneuver_lock": "waiting", "candidate_side": pending_side}
            else:
                centers, _ = self.line_control(
                    frame, lane_binary, active_mode, self.side_follow_speed, active_bias, allow_single_turn
                )
                if maneuver_locked_side is not None:
                    self.last_debug["maneuver_lock"] = maneuver_locked_side
            if crosswalk_visible:
                crosswalk_clear = 0
            else:
                crosswalk_clear += 1
            if crosswalk_clear >= self.crosswalk_clear_frames:
                entry_cleared = True
            exit_crosswalk_hits = self.update_exit_crosswalk_hits(
                entry_cleared,
                exit_crosswalk_hits,
                self.maneuver_exit_visible(self.last_stop, frame.shape[0])
                and elapsed >= self.enter_intersection_straight_time + self.intersection_min_time,
            )
            self.show_views(raw_frame, lane_binary, centers)
            if exit_crosswalk_hits >= max(1, self.exit_crosswalk_frames):
                rospy.loginfo("检测到出口横线，直接通过并恢复普通巡线")
                break
            rate.sleep()
        elapsed = rospy.get_time() - start
    ...
    def process_frame(self, frame):
        ...
        tracking_stop = self.tracking_stop_result(self.last_stop)
        tracking_visible = (
            self.state in ("APPROACH_CROSSWALK", "ALIGN_STOPLINE")
            and tracking_stop is not None
            and tracking_stop.get("confidence", 0.0) >= self.crosswalk_track_confidence
        )
        cooldown_ready = now >= self.stop_cooldown_until
        if candidate or tracking_visible:
            self.last_reliable_stop = self.last_stop if candidate else tracking_stop
            self.crosswalk_lost_count = 0
        ...
        if candidate:
            effective_stop = self.last_stop
        elif tracking_visible:
            effective_stop = tracking_stop
        else:
            effective_stop = self.last_reliable_stop
        bottom_ratio = 0.0
    ...
    def draw_debug(self, binary, centers, stop_result):
        ...
        if stop_result:
            ...
            if (
                self.state in ("APPROACH_CROSSWALK", "ALIGN_STOPLINE")
                and stop_result.get("tracking_stop_polygon")
            ):
                polygon = np.asarray(stop_result["tracking_stop_polygon"], dtype=np.int32).reshape(-1, 1, 2)
                cv2.polylines(display, [polygon], True, (0, 128, 255), 2)
        ...
        status = "state={} lock={} hint={} raw={} single={} conf={:.2f} width={:.0f} fail={}".format(
            self.state,
            self.last_debug.get("maneuver_lock"),
        ...
        if (
            stop_result
            and self.state in ("APPROACH_CROSSWALK", "ALIGN_STOPLINE")
            and stop_result.get("tracking_stop_polygon")
            and not stop_result.get("stop_polygon")
        ):
            stop_angle = stop_result.get("tracking_stop_angle_deg")
            stop_bottom_y = stop_result.get("tracking_stop_bottom_y", 0)
        else:
            stop_angle = None if not stop_result else stop_result.get("stop_angle_deg")
            stop_bottom_y = 0 if not stop_result else stop_result.get("stop_bottom_y", 0)
        stop_bottom = 0.0
        if stop_result and height > 0:
            stop_bottom = stop_bottom_y / float(height)
