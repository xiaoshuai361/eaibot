...
class LineVision:
    ...
    def _connected_stopline_candidates(self, binary, stripes, lane_lock_mask):
        ...
        if not selected:
            return []
        group_x = float(np.mean([item["center"][0] for item in selected]))
        ...
        for x1, y1, x2, y2 in lines[:, 0, :]:
            ...
            if not (min_x - width * 0.05 <= group_x <= max_x + width * 0.05):
                continue
    ...
    def _early_stripes_matching_stop(self, stop, stripes, shape):
        ...
        if not selected or stop["long_side"] < width * 0.25:
            return []
        if abs(stop["angle_deg"]) > self.stop_max_angle_deg:
            return []
        sx, sy, sw, sh = stop["box"]
        matched = []
        for stripe in selected:
