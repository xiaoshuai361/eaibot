...
class PeopleConfig:
    ...
    PT_MODEL_PATH = "/home/eaibot/robocom_ws/src/peoplebest.pt"  # 人群识别模型路径
    # 去重参数
...
class LaneFollower:
    def __init__(self):
        ...
        # 添加检测相关状态 - 只保留第四组人偶识别，时间调整为7-8秒
        self.detection_intervals = [
            {"start": 7, "end": 8, "done": False, "type": "people"}  # 只保留第四组人偶识别
        ]
        ...
        self.people_detection_count = 0  # 已完成的人群检测次数（这里固定为1次）
    ...
    def process_frame(self):
            ...
            if interval["start"] <= elapsed_time < interval["end"] and not interval["done"]:
                ...
                if ret_doll:
                    ...
                    if interval["type"] == "people":
                        # 固定为第四组人偶，即D区域
                        self.people_detection_count = 4  # 直接设置为4，确保输出为D区域
                        area = chr(64 + self.people_detection_count)  # 64是ASCII码中'A'的前一位，4→D
                        ...
                        # 检测完成后进入慢速模式
                        self.slow_mode = True
                        rospy.loginfo("D区域人群检测完成，进入慢速模式")
        ...
        if self.slow_mode and not self.ramp_mode and not self.ramp_mode_completed:
            ...
            # 检查是否检测到双黑线
            if abs(deviation) < 3:  # 偏差小于3像素，认为检测到双黑线
