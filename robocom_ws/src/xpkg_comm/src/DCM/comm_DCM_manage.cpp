...
void CommDCMManage::DataConvert()
{
    ...
    static unsigned int checksum_error_streak = 0;
    t_MsgCan can_buff;
    int i;
    ...
    for(i=0;i<m_data_recv.size();i++)
    {
        ...
        frame_len = m_data_recv.at(i+1);
        if(m_data_recv.at(i) != 0x55 || frame_len <8 || frame_len >16)continue;  //head cheak
        if(m_data_recv.size()-i < frame_len)break;//broken data
        if(DataCheckSum(m_data_recv.data()+i) != m_data_recv.at(i + frame_len - 1))   //sum cheak
        {
            checksum_error_streak++;
            if(checksum_error_streak > 1)
            {
                data_err_count++;
        ...
        checksum_error_streak = 0;

        memset(&can_buff,0,sizeof(can_buff));
        can_buff.len = frame_len - 8;
