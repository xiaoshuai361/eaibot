...
bool CommCIFCOM::COMOpen(const std::string& device)
{
    if(device.empty()) return COMOpen();
    if (COMIsOpen()==true)return false;

    m_COM_fd = open(device.c_str(),O_RDWR | O_NOCTTY | O_NONBLOCK);
    if(m_COM_fd!=-1)
    {
        #ifdef ROS
        ROS_INFO("xnode_comm: COM device %s",device.c_str());
        #else
        printf("COM device : %s",device.c_str());
        #endif
        return true;
    }

    #ifdef ROS
    ROS_ERROR("xnode_comm: Open COM failed on %s",device.c_str());
    #else
    printf("Open COM failed on %s",device.c_str());
    #endif
    return false;
}
...
bool CommCIFCOM::COMInitDefault(const std::string& device)
{
    if(COMOpen(device) == false)return false;
    if(COMSet(460800,0,8,1,'n') == false)return false;
    tcflush(m_COM_fd, TCIOFLUSH); //flush IO quene
    return true;
}
